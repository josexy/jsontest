...
public:
    ...
    friend std::ostream &operator<<(std::ostream &__out, JsonValue *jsonValue) {
        if (jsonValue) __out << jsonValue->toString();
        return __out;
    }

    friend std::ostream &operator<<(std::ostream &__out, JsonValue &jsonValue) {
        __out << jsonValue.toString();
        return __out;
    }
