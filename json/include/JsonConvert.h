#ifndef HELLOWORLD_JSONCONVERT_H
#define HELLOWORLD_JSONCONVERT_H
...
public:
    ...
    explicit JsonConvert(const JsonParser &jsonParser, bool format = false, int indent = 4, bool escape = false)
        ...
        if (!jsonParser.__parser) const_cast<JsonParser &>(jsonParser).parseJson();
...
private:
    std::shared_ptr<JsonValue> __jsonValuePtr;
    std::vector<std::string> __lstJsonValue;
    bool __format;
    int __ptr, __len, __indent;
};

#endif //HELLOWORLD_JSONCONVERT_H
