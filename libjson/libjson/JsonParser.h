...
class JsonParser {
        ...
        if (token->tag() == Tag::JSON_OBJECT_BEGIN) {
            ...
            if (lexer.token_list.size() == 1) {
                switch (token->tag()) {
                    ...
                    default:
                        throw JsonSyntaxError(token->line(), token->col(),
                                              "invalid token");
                        break;
                }
            } else {
   ...
   protected:
    ...
    void handle_after_object_array_closure_exception(Token* curToken) {
        // ]:  ][  ]{   }:  }[  }{
        if (__tokenIndex < lexer.token_list.size()) {
    ...
    void handle_key_not_string_exception(Token* curToken) {
        if (curToken->tag() != Tag::JSON_STRING)
            throw JsonSyntaxError(
                ...
                "object key is not string type " + curToken->toString());
    }
        ...
        if (__tokenIndex < lexer.token_list.size()) {
            ...
            if (nextToken->tag() != Tag::JSON_KEY_VALUE_SEP)
                throw JsonSyntaxError(
                    curToken->line(), curToken->col() + curToken->len() - 1,
                    "between" + curToken->toString() + " and " +
                        nextToken->toString() + " missing : separator");
        }
    }
    ...
    while (__tokenIndex < __tokenLength) {
        ...
        switch (value->tag()) {
            case Tag::JSON_OBJECT_BEGIN: {  // {
                ...
                std::shared_ptr<JsonObject> object_ptr =
                    make_shared<JsonObject>();
            ...
            case Tag::JSON_FLOAT:
                value_ptr =
                    make_shared<JsonNumber>(value->value<double>(), false);
                break;
            case Tag::JSON_STRING:
                value_ptr = make_shared<JsonString>(value->value<std::string>(),
                                                    lexer.escape);
                break;
            case Tag::JSON_NULL:
