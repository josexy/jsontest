...
class JsonArray : public JsonValue {
   public:
    ...
    std::shared_ptr<JsonValue> get(int index) {
        if (index < 0 || index >= __lstValue.size()) return nullptr;
    ...
    auto add(JsonValue* value) {
        return add(std::shared_ptr<JsonValue>(value));
    }
    auto add() { return add(std::make_shared<JsonNull>()); }
    auto add(int value) {
        return add(std::make_shared<JsonNumber>(value, true));
    }
